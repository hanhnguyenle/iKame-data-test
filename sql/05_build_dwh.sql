...
--
-- raw.session_start is NOT 1 row per user/day as originally assumed —
-- investigation found users with multiple rows per (user_id, event_date)
-- where session_count DIFFERS between rows (e.g. 3 vs 6), and geo
-- columns (country/tier/city) also differ within the same day — the
-- same VPN/multi-device pattern already seen and handled for
-- first_open (see docs/data_quality_log_technical.md section 2a-bis).
-- Unlike first_open, there is no event_timestamp here to prove these
-- are the "same event" duplicated, and the differing session_count
-- values indicate genuinely separate recording contexts (e.g. two
-- devices) rather than a repeated identical event — so rows are
-- SUMmed per (user_id, event_date), not deduped down to one row.
------------------------------------------------------------
...
SELECT
    ...
    x.event_date,
    x.session_count,
    DATEDIFF(DAY, u.install_date, x.event_date) AS days_since_install
INTO dwh.fact_session_daily
FROM (
    SELECT
        s.user_id,
        TRY_CAST(s.event_date AS DATE) AS event_date,
        SUM(TRY_CAST(s.event_count AS INT)) AS session_count
    FROM raw.session_start s
    GROUP BY s.user_id, TRY_CAST(s.event_date AS DATE)
) x
LEFT JOIN dwh.dim_user u ON u.user_id = x.user_id;
GO

------------------------------------------------------------
-- DWH.FACT_ENGAGEMENT_DAILY
-- Grain: 1 row per user_id per event_date.
-- Same finding as fact_session_daily above — raw.user_engagement has
-- multiple rows per (user_id, event_date) with differing
-- engagement_time values; summed per user/day rather than deduped.
...
FROM (
    SELECT
        ...
        SUM(TRY_CAST(e.engagement_time AS DECIMAL(18,3))) AS engagement_time_sec
    FROM raw.user_engagement e
    GROUP BY e.user_id, TRY_CAST(e.event_date AS DATE)
) x
LEFT JOIN dwh.dim_user u ON u.user_id = x.user_id;
GO
...
-- (deduped — see docs/data_quality_log_technical.md).
--
-- tier backfill: 2,336 rows have a NULL tier in raw.ad_impression.
-- Investigation found only 7 of those rows can be recovered from
-- dim_user.tier (COALESCE below); the remaining 2,329 belong to
-- users with no dim_user record at all (the left-censoring case —
-- see docs/data_quality_log_technical.md section 6, Phát hiện 4:
-- users who installed before the 2023-06-01 tracking window). Those
-- rows keep tier = NULL and will not match mart.dim_ecpm, so they
-- are correctly excluded from any eCPM-based revenue calculation —
-- there is no way to know their tier.
------------------------------------------------------------
...
SELECT
    ...
    COALESCE(NULLIF(LTRIM(RTRIM(a.tier)), ''), u.tier) AS tier,
